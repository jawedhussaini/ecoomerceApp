...
export default function OrderDetails() {
  ...
  async function extractOrerDetails() {
    const res = await getOrderDetails(parmas["orderDetails"]);
    if (res.success) {
  ...
  useEffect(() => {
    if (user !== null) extractOrerDetails();
  }, [user]);

  console.log(orderDetails);
  return (
    <div className="py-14 bg-white px-4 md:px-6">
      ...
      <div className="mt-10 flex flex-col justify-center xl:flex-row items-stretch w-full xl:space-x-8 md:space-y-6 xl:space-y-0">
        <div className="flex flex-col justify-start items-start w-full space-y-4 md:space-y-6 xl:space-y-8">
          <div className="flex flex-col justify-start items-start bg-gray-50 px-4 py-4 md:p-6 xl:p-8 w-full">
            <p className="font-bold text-lg uppercase">your order details</p>
            {orderDetails &&
              ...
              ? orderDetails.orderItems.map((item) => (
                  ...
                  >
                    <div className="pb-4 md:pb-8 w-full md:w-40">
                      <img
                        src={item && item.product && item.product.imgUrl}
                        className="w-full hidden md:block"
                      />
                    </div>
                    <div className="border-b border-gray-300 md:flex-row flex flex-col justify-between items-start w-full pb-8 space-y-4 md:space-y-0">
          ...
          <div className="flex justify-center flex-col md:flex-row items-stretch w-full space-y-4 md:space-y-0 md:space-x-5 xl:space-x-8">
            <div className="flex flex-col px-4 py-6 md:p-6 xl:p-8 w-full bg-gray-50 space-y-6">
              ...
              <div className="flex justify-center items-center w-full space-y-4 flex-col border-gray-200 border pb-4">
       ...
       <div className="flex flex-col gap-5">
         <div className="bg-gray-50 w-full xl:w-96 flex items-center md:items-start px-4 py-6 flex-col">
          ...
          <div className="flex flex-col justify-start items-start flex-shrink-0">
            <div className="flex flex-col gap-4 w-full md:justify-start py-8 border-b border-gray-200">
              <p className="text-xl font-semibold leading-4 text-left text-gray-950">
                Name: {user?.name}
              </p>
              <p className="text-xl font-semibold leading-4 text-left text-gray-950">
                Email: {user?.email}
        ...
        <div className="flex justify-between xl:h-full items-stretch w-full flex-col mt-6 md:mt-0">
          <div className="flex justify-center md:justify-start xl:flex-col flex-col md:space-x-6 lg:space-x-8 xl:space-x-0 space-y-4 md:space-y-0 xl:space-y-12 md:flex-row items-center md:items-start">
            <div className="flex justify-center md:justify-start items-center md:items-start flex-col space-y-4 xl:mt-8">
              ...
              <p>Name: {orderDetails && orderDetails.shippingAddress.name}</p>
            ...
            <button
                          onClick={()=>router.push('/')}
                            type="button"
                            className="mt-3 w-full mr-5 group inline-flex items-center justify-center bg-black px-6 py-4 text-lg text-white font-medium uppercase tracking-wide"
                          >
                            Shop Again
                          </button>
          </div>
