    ...
    <div onClick={() => router.push(`/product/${item._id}`)}>
      <div className="overflow-hideen aspect-w-1 aspect-h-1 h-52">
        <img
          src={item.imgUrl}
          alt="product Image"
          className="h-full w-full object-cover transition-all duration-300 group-hover:scalse-125"
        />
      </div>
      {item.onSale === "yes" ? (
        <div className="absolute top-0 m-2 rounded-full bg-black">
          <p className="rounded-ful p-1 text-[8px] font-bold uppercase tracking-wide text-white sm:py-1 sm:px-3">
            sale
          </p>
        </div>
      ) : null}
